...
for j=1:size(inDir,1)
    ...
    for i=1:size(fileList,1)
        ...
        if ~isempty(freqRange) % Reduce array to desired frequency range
            ...
            PSD = PSD(2:end,[1,lowInd+1:highInd+1]);
        else
            PSD = PSD(2:end,:);
        end
        ...
        if ~isempty(freqRange) % Reduce array to desired frequency range
            [m,lowInd] = min(abs(mDecBands(:,2)-freqRange(1)));
            [m,highInd] = min(abs(mDecBands(:,2)-freqRange(2)));
            mDecBands = mDecBands(lowInd:highInd,:);
        end
        mBandwidth = mDecBands(:,3)-mDecBands(:,1);
