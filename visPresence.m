...
% possibly ending times) as Matlab serial dates or datetimes.  If end time
...
%  'Effort', serial date (datenum) or datetime Matrix - Indicates where 
%      effort to detect was made.  Regions of the plot where there was 
%      no effort will be displayed with a lighter version of the plot color.
%
...
%  'Resolution_min', M - Plot resolution (bin size) in minutes (default 60)
%      (will also accept legacy Resolution_m)
...
if isdatetime(presence)
    presence = datenum(presence);
end
...
while vidx < length(varargin)
    switch varargin{vidx}
        ...
        case 'DateTickInterval'
            ...
            if ~isscalar(DateTickInterval) && ~strcmp(DateTickInterval,'month')
                error('DateTickInterval must be scalar, or month');
            end
        case 'DateRange'
            DateRange = varargin{vidx+1}; vidx=vidx+2;
            if isdatetime(DateRange)
                DateRange = datenum(DateRange);
            end
            if numel(DateRange) ~= 2 || ~ isnumeric(DateRange)
                error('DateRange requires [start, stop] matlab serial dates or datetimes');
            end
        case 'Effort'
            ...
            if isdatetime(Effort)
                Effort = datenum(Effort);  % convert to serial date
            end
        ...
        case {'Resolution_min', 'Resolution_m'}
...
if ~ isempty(Effort)
    % effort should be sorted, but in cases of overlapping effort
    % the last end date may not be the last entry
    % NOTE:  We do not check for detections in holes between effort
    % When the data have been validated in Tethys, this should not 
    % occur.
    firsteff = min(Effort(:,1));  % should not be needed
    lasteff = max(Effort(:,2));
    if ~ isempty(presence) && ...
            (presence(1) < firsteff || lasteff < presence(end,end))
        warning(['Detections outside of effort\n', ...
            '\tDetection range [%s %s]\n\tEffort range:  [%s %s]'], ...
            dbSerialDateToISO8601(presence(1)), ...
            dbSerialDateToISO8601(presence(end, end)), ...
            dbSerialDateToISO8601(firsteff), ...
            dbSerialDateToISO8601(lasteff));
    end
end
...
if ~ isempty(presence)
    ...
    if N > 0
        ...
        for idx = 1:N
            ...
            % Bar goes from y + BarOffset to y + BarOffset + BarHeight
            y1 = y + BarOffset;
            y2 = y1 + BarHeight;

            % plot the rectangle on the axis
            presenceH(idx) = patch([x; x+width(idx); x+width(idx); x], ...
                [y1; y1; y2; y2], Color, 'Parent', BarH(1), ...
                'LineStyle', LineStyle, 'FaceAlpha', Transparency);
            % store serial dates as user data (without bar offset)
            info.dates = x+y + [0 width(idx)];
            info.label = Label;
...
if isscalar(DateTickInterval)
    ...
    while (last_day>current_day)
        ytick = [ytick addtodate(current_day,1,'month')];
        current_day = addtodate(current_day,1,'month');
    end
    set(axH, 'YGrid', 'on', ...
        'YTick', ytick);
end
